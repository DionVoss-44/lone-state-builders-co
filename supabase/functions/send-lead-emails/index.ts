...
//       supabase secrets set RESEND_API_KEY=re_jbrGk2Ud_HRW43x9CkxqcLdCkBkUWdCLK
...
//  at resend.com/domains. On free tier, you can only send to your own email.
// ============================================================================
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (lead.email) {
      await resendSend({
        to: [lead.email],
        subject: `We've got your plans, ${String(lead.name).split(" ")[0]} — Lone State Services`,
